...
(define (solve in)
  ...
  (define (clamp-add x y) (max 0 (min (- num-names 1) (+ x y))))
  (define idx
    (for/fold ([idx 0]) ([instr instructions])
      (let ([dir (string-ref instr 0)]
            [num (string->number (substring instr 1))])
        (clamp-add idx (match dir
                         [#\L (- num)]
                         [#\R num])))))
  (list-ref names idx))
...
(module+ test
  ...
  (check-equal? (call-with-input-file (build-path here "example.txt") solve) "Fyrryn"))
